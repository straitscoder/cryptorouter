...
func (ok *Okx) UpdateOrderbook(ctx context.Context, pair currency.Pair, assetType asset.Item) (*orderbook.Base, error) {
	...
	var orderbookNew *OrderBookResponseDetail
	var err error
	...
	for x := range orderbookNew.Bids {
		book.Bids[x] = orderbook.Tranche{
			...
			Price:  orderbookNew.Bids[x].DepthPrice.Float64(),
		}
	}
	...
	for x := range orderbookNew.Asks {
		book.Asks[x] = orderbook.Tranche{
			...
			Price:  orderbookNew.Asks[x].DepthPrice.Float64(),
		}
	}
	err = book.Process()
	if err != nil {
...
func (ok *Okx) GetAccountFundingHistory(ctx context.Context) ([]exchange.FundingHistory, error) {
	...
	for x := range depositHistories {
		resp = append(resp, exchange.FundingHistory{
			ExchangeName:    ok.Name,
			Status:          depositHistories[x].State.String(),
...
func (ok *Okx) CancelAllOrders(ctx context.Context, orderCancellation *order.Cancel) (order.CancelAllResponse, error) {
	...
	if orderCancellation.Type != order.UnknownType && orderCancellation.Type != order.AnyType {
		oType = orderCancellation.Type.String()
		if err != nil {
			return order.CancelAllResponse{}, err
		}
	}
	...
	if len(tradesData) > 0 {
		...
		for i := range tradesData {
			...
			trades = append(trades, order.TradeHistory{
				Price:     tradesData[i].FillPrice.Float64(),
				Amount:    tradesData[i].FillSize.Float64(),
				Fee:       tradesData[i].Fee.Float64(),
				Exchange:  ok.Name,
...
allOrders:
	for {
		...
		for i := range activeOrders {
			if req.StartTime.Equal(activeOrders[i].CreationTime.Time()) ||
				activeOrders[i].CreationTime.Time().Before(req.StartTime) ||
				endTime == activeOrders[i].CreationTime.Time() {
				// reached end of orders to crawl
				break allOrders
			}
			...
			for j := range req.Pairs {
				...
				resp = append(resp, order.Detail{
					...
					Status:               orderStatus,
					AssetType:            req.AssetType,
					Date:                 activeOrders[i].CreationTime.Time(),
					LastUpdated:          activeOrders[i].UpdateTime.Time(),
					Pair:                 pair,
