...
func MarketMaker() error {
	marketMaker := &cobra.Command{
		Use:   "Market Maker",
		...
		Loop:
			for {
				...
				case "3":
					...
					continue Loop
				case "4":
					if err := fixEngine.SecuritiesDetail(); err != nil {
						log.Println(err)
						break
					}
					continue
...
func Menu() (string, error) {
	...
	fmt.Println("4. Security Detail Request")
	fmt.Println("0. Exit")
...
func SecReqId() string {
	return stringField("Security Request ID")
}
...
func TimeInForce() enum.TimeInForce {
	fmt.Println("You can choose either GTC or IOC")
	tifStr := stringField("Time in Force")
	return convertTIF(tifStr)
}
