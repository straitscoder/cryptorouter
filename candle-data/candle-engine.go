...
func New() (*CandleDataEngine, error) {
	newEngineMutex.Lock()
	defer newEngineMutex.Unlock()

	var t CandleDataEngine
...
func NewFromSetting(settings *Settings, flagset map[string]bool) (*CandleDataEngine, error) {
	newEngineMutex.Lock()
	defer newEngineMutex.Unlock()
	if settings == nil {
...
func (te *CandleDataEngine) Start() error {
	if te == nil {
	...
	te.CandleStore, err = NewCandleStore(te.ExchangeManager, te.websocketRoutineManager)
	if err != nil {
		gctlog.Errorf(gctlog.Global, "Unable to initiate Candle Engine: %+v", err)
	} else {
		go te.CandleStore.Start()
	}
...
func (tradeEngine *CandleDataEngine) Stop() {
	newEngineMutex.Lock()
	defer newEngineMutex.Unlock()
...
func (tradeEngine *CandleDataEngine) GetExchangeByName(exchName string) (exchange.IBotExchange, error) {
	return tradeEngine.ExchangeManager.GetExchangeByName(exchName)
}

// UnloadExchange unloads an exchange by name
func (tradeEngine *CandleDataEngine) UnloadExchange(exchName string) error {
...
func (tradeEngine *CandleDataEngine) GetExchanges() []exchange.IBotExchange {
...
func (tradeEngine *CandleDataEngine) LoadExchange(name string, wg *sync.WaitGroup) error {
...
func (tradeEngine *CandleDataEngine) SetupExchanges() error {
